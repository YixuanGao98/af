...
class HyperIQASolver(object):
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            if test_srcc > best_srcc:
                # best_loss = (sum(epoch_loss) / len(epoch_loss))
                best_srcc = test_srcc
                best_plcc = test_plcc
                best_rmse =test_rmse
                torch.save(self.model_hyper.state_dict(), "hyperIQA_live.pth")
        ...
        print('Best test SRCC %f, PLCC %f, rmse %f' % (best_srcc, best_plcc,best_rmse))

        return best_srcc, best_plcc,best_rmse
    ...
    def test(self, data):
        ...
        test_rmse=np.sqrt((((pred_scores)-np.array(gt_scores))**2).mean())

        # print('test SRCC %f, PLCC %f, rmse %f' % (test_srcc, test_plcc,test_rmse))
