...
class DataLoader(object):
    ...
    def __init__(self, dataset, path, img_indx, patch_size, patch_num, batch_size=1, istrain=True):
        ...
        if (dataset == 'live') | (dataset == 'csiq') | (dataset == 'TID') | (dataset == 'livec')| (dataset == 'LIVEMD')| (dataset == 'cid')| (dataset == 'VCLFER')| (dataset == 'SPAQ'):
            # Train transforms
            if istrain:
        ...
        if dataset == 'live':
            ...
            self.data = folders.TIDFolder(
                root=path, index=img_indx, transform=transforms, patch_num=patch_num)
        elif dataset == 'LIVEMD':
            self.data = folders.LIVEMDFolder(
                root=path, index=img_indx, transform=transforms, patch_num=patch_num)
        elif dataset == 'cid':
            self.data = folders.CID2013Folder(
                root=path, index=img_indx, transform=transforms, patch_num=patch_num)
        elif dataset == 'VCLFER':
            self.data = folders.VCLFERFolder(
                root=path, index=img_indx, transform=transforms, patch_num=patch_num)
        elif dataset == 'SPAQ':
